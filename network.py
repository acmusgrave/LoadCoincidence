...
pp.create_line_from_parameters(net, from_bus=1, to_bus=2, length_km=1, r_ohm_per_km=z_base*rt, x_ohm_per_km=z_base*xt, c_nf_per_km=0, max_i_ka=1.0, index=-1)
net.line.index = net.line.index+1
...
A[0, 0] = -1
for i in range(1, n):
    A[i, net.line.iloc[i-1].from_bus-1]=1
    A[i, net.line.iloc[i-1].to_bus-1]=-1
...
Dr = (1/z_base)*np.diag(net.line.r_ohm_per_km*net.line.length_km)
R = F @ Dr @ F.T
print(R)
...
Dx = (1/z_base)*np.diag(net.line.x_ohm_per_km*net.line.length_km)
X = F @ Dx @ F.T
print(X)
